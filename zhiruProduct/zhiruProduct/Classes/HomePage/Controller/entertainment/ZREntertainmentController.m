...
- (void)homeBaseViewControllerHeaderClickWithBtn: (NSInteger) index{
    ...
    [CustomHudView show];
    //跳转
    [ZRHomePageRequst requestGetYuleIndexWithLongitude :_longitude andLatitude:_latitude andLabel:labelModel.nav_id andSuccess:^(id success) {
...
- (void)startRefresh{
    ...
    ZRUserAddress * address = [ZRUserAddress sharedInstance];
    WS(ws)
    [ZRHomePageRequst requestGetYuLeListWithLongitude:address.Longitude andLatitude:address.Latitude andRegionId:region.region_id andCity:@""  andLabel:label.nav_id andSort:sort andScreen:screen andRows:[NSString stringWithFormat:@"%d",ZRRows]  andPage:[NSString stringWithFormat:@"%ld",_page ] andSuccess:^(id success) {
