...
//        _collectionView.backgroundColor = [UIColor redColor];
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, 220, screenSize.width, screenSize.height)];
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (leftTableView) {
                ...
                if (_isNavClick == YES) {
                    ...
                    leftTableView.frame = CGRectMake(_origin.x, _orginY + self.frame.size.height -45 , self.frame.size.width*ratio, tableViewHeight);
                }
            }
            if (rightTableView) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (leftTableView) {
                ...
                leftTableView.frame = CGRectMake(_origin.x, _orginY + self.frame.size.height, self.frame.size.width*ratio, 0);
            }
            if (rightTableView) {
                rightTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*(1-ratio), 0);
            }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:numberOfRowsInColumn:leftOrRight: leftRow:)]) {
        NSLog(@"%ld",(long)self.currentSelectedMenudIndex);
