...
@property (nonatomic , strong) NSMutableArray  * homeBtnTitleArr;
@property (nonatomic , strong) NSMutableArray  * homeBtnImg;

@property (nonatomic , strong) NSMutableArray * lunBoModelMarr;
@end

@implementation ZRHomeHeadView

-(NSMutableArray *)lunBoModelMarr{
    if (_lunBoModelMarr == nil) {
        _lunBoModelMarr = [NSMutableArray array];
    }
    return _lunBoModelMarr;
}

-(void)setModel:(ZRHomeDataModel *)model{
    _model = model;
    
    NSArray * boutiqueArr = model.boutiqueTourism;
    
    for (int i = 0; i< boutiqueArr.count; i++) {
        ZRCircleImgModel * LunboModel = boutiqueArr[i];
        if (LunboModel.img != nil) {
            [self.lunBoModelMarr addObject:LunboModel.img];
        }
        
    }

    
    [self createAdvert:_lunBoModelMarr];
    
    [self createBtnView];
}

-(NSMutableArray *)homeBtnTitleArr{
    if (_homeBtnTitleArr == nil) {
        _homeBtnTitleArr = [NSMutableArray array];
        
        NSArray * mainArr = _model.mainNav;
        
        for (ZRNavModel * navModel in mainArr) {
            [_homeBtnTitleArr addObject:navModel.nav_name];
        }
    }
    return _homeBtnTitleArr;
}

-(NSMutableArray *)homeBtnImg{
    if (_homeBtnImg == nil) {
        _homeBtnImg = [NSMutableArray array];
        
        NSArray * mainArr = _model.mainNav;
        
        for (ZRNavModel * navModel in mainArr) {
            [_homeBtnImg addObject:navModel.img_url];
        }
    }
    return _homeBtnImg;
}
...
- (void)createBtnView{
    ...
    for (int i =0 ; i< self.homeBtnTitleArr.count; i++) {
        ...
        [btn sd_setImageWithURL:self.homeBtnImg[i] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"xiaolian-huang"]];
