...
- (void)setModel:(ZRSuperOderModel *)model
{
    ...
    if ([_model.process isEqualToString:@"21"]) {
    ...
    } else if ([_model.process isEqualToString:@"22"]) {
        [self.orderState setText:@"订单进行中"];
        [self.commentButton setHidden:YES];
        [self.againButton setTitle:@"再来一单" forState:UIControlStateNormal];
    } else if ([_model.process isEqualToString:@"23"]) {
        [self.orderState setText:@"订单进行中"];
        [self.commentButton setHidden:YES];
        [self.againButton setTitle:@"再来一单" forState:UIControlStateNormal];
    } else if ([_model.process isEqualToString:@"25"]) {
        [self.orderState setText:@"订单已取消"];
        [self.commentButton setHidden:YES];
        [self.againButton setTitle:@"再来一单" forState:UIControlStateNormal];
    } else if ([_model.process isEqualToString:@"26"]) {
        [self.orderState setText:@"订单已完成"];
        [self.commentButton setHidden:YES];
        [self.againButton setTitle:@"再来一单" forState:UIControlStateNormal];
    } else if ([_model.process isEqualToString:@"24"]) {
        [self.orderState setText:@"订单已取消"];
        [self.commentButton setHidden:YES];
        [self.againButton setTitle:@"再来一单" forState:UIControlStateNormal];
    }
