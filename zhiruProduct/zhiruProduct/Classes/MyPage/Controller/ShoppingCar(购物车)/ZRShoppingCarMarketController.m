        ...
        if (self.allArray.count == 0) {
            ...
            if (self.allMoney >= 20.0) {
                ...
                [ZRUserInterfaceModel userFineDefaultAddressCallBack:^(id result) {
                    ...
                    ZRMarketModel * marketModel = [self.dataArray objectAtIndex:0];
                    [ZRSupermarketRequest requestGetDustabceWithLongitudeOne:marketModel.longitude andLatitudeOne:marketModel.latitude andLongitudeTwo:longitude andLatitudeTwo:latitude andSuccess:^(id success) {
...
//                        _superModel.kaName = model.kaName;
                        ...
                        confirmOrderVC.latitude = marketModel.latitude;
                        confirmOrderVC.longitude = marketModel.longitude;
                        confirmOrderVC.distanceStr = distanceStr;
                        confirmOrderVC.weather = _weather.floatValue;
                        
                        self.superModel.kaName = marketModel.kaName;
                        ZRMarketDetailModel * detailModel1 = [self.allArray lastObject];
                        self.superModel.kaId = detailModel1.kaId;
                        confirmOrderVC.supermarketHomeModel = self.superModel;
                        confirmOrderVC.superOrderState = @"1";
                        ...
                        for (ZRMarketDetailModel * detailModel in self.allArray) {
                            ...
                            for (int i = 0; i < detailModel.num.integerValue; i++) {
                                ...
                                model.now_price = (NSNumber *)detailModel.nowPrice;
                        ...
                        NSInteger allNumber;
                        for (ZRMarketDetailModel * detailModel in self.allArray) {
                            allNumber += detailModel.num.integerValue;
                        }
                        [ZRSupermarketHomeObj shareInstance].allProductsArray = array;
                        [ZRSupermarketHomeObj shareInstance].allPrice = self.allMoney;
                        [ZRSupermarketHomeObj shareInstance].allNumber = allNumber;
                        confirmOrderVC.productArr = [ZRSupermarketHomeObj shareInstance].allProductsArray;
                    } andFailure:^(id error) {
                        [CustomHudView dismiss];
                    }];
...
- (void)deleteMarketNumSection:(NSInteger)section andRow:(NSInteger)row
{
    ...
    if (num == 1) {
        ...
        num--;
        detailModel.num = [NSString stringWithFormat:@"%ld", (long)num];
