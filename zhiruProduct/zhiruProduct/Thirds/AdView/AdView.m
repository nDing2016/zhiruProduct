...
//    UILabel * _adLabel;
    ...
    //用于确定滚动式由人导致的还是计时器到了,系统帮我们滚动的,YES,则为系统滚动,NO则为客户滚动(ps.在客户端中客户滚动一个轮播图后,这个轮播图的计时器要归0并重新计时)
    BOOL _isTimeUp;
    //为每一个图片添加一个轮播图语(可选)
...
@property (nonatomic, assign) NSInteger adViewTag;
...
//超市专用
+ (id)adScrollViewWithFrame:(CGRect)frame bundleImageLinkURL:(NSArray *)imageLinkURL pageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle
{
    
    AdView * adView = [[AdView alloc]initWithFrame:frame];
    adView.adViewTag = 10000;
    adView.placeHoldImage = [UIImage imageNamed:@""];
    [adView setBundleImageLinkURL:imageLinkURL];
    [adView setPageControlShowStyle:PageControlShowStyle];
...
#pragma mark - 设置轮播图所使用的图片(名字)
- (void)setBundleImageLinkURL:(NSArray *)imageLinkURL
{
    _imageLinkURL = imageLinkURL;
    if (imageLinkURL.count == 1) { //一张图
...
//    [_leftImageView sd_setImageWithURL:[NSURL fileURLWithPath:imageLinkURL[leftImageIndex]] placeholderImage:self.placeHoldImage];
//    [_centerImageView sd_setImageWithURL:[NSURL fileURLWithPath:imageLinkURL[centerImageIndex]] placeholderImage:self.placeHoldImage];
//    [_rightImageView sd_setImageWithURL:[NSURL fileURLWithPath:imageLinkURL[rightImageIndex]]placeholderImage:self.placeHoldImage];
    
    [_leftImageView setImage:imageLinkURL[leftImageIndex]];
    [_centerImageView setImage:imageLinkURL[centerImageIndex]];
    [_rightImageView setImage:imageLinkURL[rightImageIndex]];
...
- (void)setAdTitleArray:(NSArray *)adTitleArray withShowStyle:(AdTitleShowStyle)adTitleStyle
{
    ...
    //上面的灰色遮罩
    UIView *vv = [[UIView alloc] initWithFrame:CGRectMake(0, kAdViewHeight - 40, kAdViewWidth, 20)];
    ...
    _centerAdLabel.frame = CGRectMake(0, kAdViewHeight - 40, kAdViewWidth, 20);
...
{    
    ...
    if (self.adViewTag == 10000) {
        [_leftImageView setImage:_imageLinkURL[leftImageIndex]];
        [_centerImageView setImage:_imageLinkURL[centerImageIndex]];
        [_rightImageView setImage:_imageLinkURL[rightImageIndex]];
...
{
    if (_callBack) {
        _callBack(centerImageIndex,_imageLinkURL[centerImageIndex],_centerImageView);
